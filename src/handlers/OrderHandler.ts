...
export class OrderHandler {
    ...
    async updateOrder(id: number, order: IOrder, userId: number): Promise<IHandlerReturn> {
        try {
    ...
    async updateOrderStatus(id: number, status: "pending" | "completed" | "canceled", userId: number): Promise<IHandlerReturn> {
        try {
            const userOrder = await this.orderService.getOrder(id);
            if (!userOrder) {
                return {
